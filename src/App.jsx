...
const styles = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .header-badge-accent {
    ...
    border: 1px solid var(--accent-dim); padding: 6px 14px; border-radius: 100px;
    letter-spacing: 0.1em; background: var(--accent-dim);
  }

  .left-col { grid-column: 1; grid-row: 2; padding-bottom: 60px; }
  .right-col { grid-column: 2; grid-row: 2; padding-bottom: 60px; position: sticky; top: 40px; height: fit-content; }
  ...
  input[type=range]::-webkit-slider-thumb {
    ...
    box-shadow: 0 0 12px var(--accent-glow); transition: transform 0.15s;
  ...
  .sex-btn {
    padding: 12px; text-align: center; cursor: pointer; font-size: 0.8rem;
  ...
  .info-box {
    background: var(--accent-dim); border: 1px solid rgba(217,79,43,0.2);
    border-radius: 10px; padding: 12px 16px; font-size: 0.75rem;
    color: var(--text-muted); line-height: 1.6; margin-top: 4px;
  }
  ...
  .cta {
    ...
    letter-spacing: 0.03em; transition: all 0.2s; margin-top: 8px;
  }
  ...
  /* Results */
  .results-empty {
    border: 1.5px dashed var(--border); border-radius: 16px; padding: 52px 32px;
  ...
  .results-panel {
    ...
    background: var(--surface); animation: fadeUp 0.4s ease;
  ...
  .results-header p { font-size: 0.67rem; color: var(--text-muted); font-family: var(--font-mono); letter-spacing: 0.12em; text-transform: uppercase; margin-bottom: 8px; }
  ...
  .imc-label { font-size: 0.72rem; color: var(--text-muted); font-family: var(--font-mono); }
  .imc-val { font-family: var(--font-mono); font-size: 0.85rem; font-weight: 500; }
  .imc-cat { font-size: 0.68rem; padding: 2px 8px; border-radius: 100px; font-family: var(--font-mono); }
  ...
  .target-row {
    background: var(--bg-warm); border-radius: 10px; padding: 12px 16px;
  ...
  .macro-card { background: var(--bg-warm); border-radius: 10px; padding: 14px 10px; text-align: center; border: 1px solid var(--border); }
  ...
  .extras { padding: 16px 28px; border-bottom: 1px solid var(--border); display: flex; flex-direction: column; gap: 10px; }
  .extra-row { display: flex; justify-content: space-between; align-items: center; }
  .extra-label { font-size: 0.78rem; color: var(--text-muted); }
  .extra-val { font-family: var(--font-mono); font-size: 0.9rem; font-weight: 500; }

  .note { padding: 14px 28px 22px; font-size: 0.7rem; color: var(--text-dim); line-height: 1.7; font-style: italic; }

  .footer {
    grid-column: 1 / -1; padding: 24px 0;
...
// Calculates left% position for a slider mark given its value and the slider min/max
function markPos(val, min, max) {
  const thumbHalf = 9; // half of 18px thumb
  const pct = (val - min) / (max - min);
  // CSS trick to account for thumb width at edges
  return `calc(${pct * 100}% - ${thumbHalf * (pct * 2 - 1)}px)`;
}

function SliderWithMarks({ label, value, onChange, min, max, step = 1, unit = "", marks = [], hint }) {
  const pct = (value - min) / (max - min);
  const gradient = `linear-gradient(to right, var(--accent) ${pct * 100}%, var(--surface-2) ${pct * 100}%)`;

  return (
    <div className="slider-field">
      <div className="slider-top">
        <label>{label}</label>
        <span className="slider-value">{typeof value === "number" && unit === "pasos" ? value.toLocaleString() : value}{unit !== "pasos" ? " " + unit : ""}</span>
      </div>
      <div className="slider-wrap">
        ...
        {marks.map(m => (
          <span
            ...
            style={{ left: markPos(m.val, min, max) }}
          >
            {m.label}
          </span>
        ))}
...
function imcInfo(imc) {
  if (imc < 18.5) return { label: "Bajo peso", color: "#3a6e9e", bg: "rgba(58,110,158,0.1)" };
  if (imc < 25)   return { label: "Normal", color: "#5a8a4a", bg: "rgba(90,138,74,0.1)" };
  if (imc < 30)   return { label: "Sobrepeso", color: "#e8793a", bg: "rgba(232,121,58,0.1)" };
  return           { label: "Obesidad", color: "#d94f2b", bg: "rgba(217,79,43,0.1)" };
}
...
export default function App() {
  ...
  const calcular = () => {
    ...
    let proteinG, kcalObj;
    if (objetivo === "deficit")          { kcalObj = deficit_mod; proteinG = Math.round(peso * 2.2); }
    else if (objetivo === "deficit_agresivo") { kcalObj = deficit_agr; proteinG = Math.round(peso * 2.4); }
    else if (objetivo === "mantenimiento")    { kcalObj = mantenimiento; proteinG = Math.round(peso * 1.8); }
    else if (objetivo === "superavit")        { kcalObj = superavit; proteinG = Math.round(peso * 2.0); }
    else                                      { kcalObj = superavit_agr; proteinG = Math.round(peso * 2.0); }
    ...
    const fibra = sexo === "hombre" ? 38 : 25; // g/día (Institute of Medicine)
    const imc = +(peso / ((altura / 100) ** 2)).toFixed(1);

    setResultado({
      bmr: Math.round(bmr), eat: Math.round(eatDia + cardioKcal),
      neat: Math.round(neat), tef: Math.round(tef),
      tdee: mantenimiento, deficit_mod, deficit_agr, superavit, superavit_agr,
      kcalObj, proteinG, fatG, carbG, agua, fibra, imc,
  ...
  const objetivoLabel = {
    ...
    mantenimiento: "mantenimiento", superavit: "superávit ligero",
      ...
      <div className="page">
        ...
        <header className="header">
          ...
          <div className="header-right">
            <span className="header-badge">TDEE CALCULATOR v2.1</span>
        ...
        <main className="left-col">
          ...
          <div className="section">
            ...
            <div style={{display:"flex", flexDirection:"column", gap:28}}>
              <SliderWithMarks
                label="Días por semana" value={diasFuerza} onChange={setDiasFuerza}
                min={1} max={7} unit="días"
                marks={[1,2,3,4,5,6,7].map(d => ({ val: d, label: String(d) }))}
              />
              <SliderWithMarks
                label="Duración por sesión" value={duracion} onChange={setDuracion}
                min={20} max={180} step={5} unit="min"
              ...
              <div className="field">
                ...
                <select className="styled-select" value={intensidad} onChange={e => setIntensidad(e.target.value)}>
                  ...
                  <option value="muy_intensa">Muy intensa — powerlifting / CrossFit / HIIT con pesas, esfuerzo máximo</option>
                </select>
              </div>
            </div>
          </div>
          ...
          <div className="section">
            ...
            <div className="field">
              ...
              <select className="styled-select" value={cardio} onChange={e => setCardio(e.target.value)}>
                ...
                <option value="moderado">Moderado — 3 sesiones/semana de 30-45 min (carrera, bici, elíptica)</option>
                <option value="bastante">Bastante — 4 sesiones/semana de 45-60 min o sesiones largas</option>
                <option value="mucho">Mucho — 5+ sesiones/semana o cardio de más de 60 min (running, ciclismo)</option>
              </select>
            </div>
          </div>
          ...
          <div className="section">
            ...
            <div style={{display:"flex", flexDirection:"column", gap:28}}>
              <SliderWithMarks
                label="Pasos diarios promedio" value={pasos} onChange={setPasos}
                min={1000} max={30000} step={500} unit="pasos"
                marks={[1000,5000,10000,15000,20000,25000,30000].map(d => ({ val: d, label: d >= 1000 ? (d/1000)+"k" : d }))}
              ...
              <div className="field">
                ...
                <select className="styled-select" value={trabajo} onChange={e => setTrabajo(e.target.value)}>
                  ...
                  <option value="ligero">Ligero — sentado 4-6 h, de pie o caminando el resto (dependiente, recepcionista)</option>
                  ...
                  <option value="muy_activo">Muy activo — esfuerzo físico muy exigente todo el día (construcción, mudanzas, agricultor)</option>
                </select>
              </div>
            </div>
          </div>
          ...
          <div className="section">
            ...
            <div className="field">
              ...
              <select className="styled-select" value={objetivo} onChange={e => setObjetivo(e.target.value)}>
                <option value="deficit_agresivo">Pérdida de grasa agresiva — déficit de 500 kcal (~0.5 kg/semana)</option>
                ...
                <option value="superavit">Volumen limpio — superávit de 250 kcal (mínima grasa)</option>
                <option value="superavit_agresivo">Volumen agresivo — superávit de 500 kcal (~0.5 kg/semana)</option>
              </select>
            </div>
          </div>
          ...
          {!resultado ? (
            ...
            <div className="results-panel">
              ...
              <div className="macros">
                <div className="macros-title">Macros recomendados · {objetivoLabel}</div>
              ...
              <div className="extras">
                <div className="extra-row">
                  <span className="extra-label">💧 Agua recomendada al día</span>
                  <span className="extra-val" style={{color:"#3a6e9e"}}>{resultado.agua} L</span>
                </div>
                <div className="extra-row">
                  <span className="extra-label">🌾 Fibra dietética mínima</span>
                  <span className="extra-val" style={{color:"#5a8a4a"}}>{resultado.fibra} g</span>
                </div>
              </div>

              <p className="note">
                Estimación con margen ±10–15%. Pésate en las mismas condiciones y ajusta cada 2–3 semanas según evolución real del peso. El IMC es orientativo; no distingue masa muscular de grasa.
              </p>
            </div>
          )}
        </aside>
